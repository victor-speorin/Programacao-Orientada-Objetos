...
public class LanceService {
    ...
    // valor do último lance. Em ambos os casos é preciso salvar em produto o
        ...
        if (lance.getProduto().getLances().isEmpty()) {
            if(lance.getValor() < lance.getProduto().getLanceMinimo()) {
                throw new ValorDeLanceInvalidoException(
                    "O valor do lance (" + lance.getValorMasc() + ") deve ser maior ou igual " +
                    "ao valor do lance mínimo (" + lance.getProduto().getLanceMinimoMasc() + ")");
            }
        }
        else {
            if (lance.getValor() <= lance.getProduto().getValorUltimoLance()) {
                throw new ValorDeLanceInvalidoException(
                    "O valor do lance (" + lance.getValorMasc() + ") deve ser maior " +
                    "do que o valor do último lance (" + lance.getProduto().getValorUltimoLanceMasc() + ")");
            }
        }
    ...
    public Lance remover(int id) {
        Lance lance = recuperarLancePorId(id);
        lance.getProduto().getLances().remove(lance);
        if (lance.getProduto().getLances().isEmpty()) {
            lance.getProduto().setValorUltimoLance(0);
        }
        else {
            int tamanho = lance.getProduto().getLances().size();
            Lance ultimoLance = lance.getProduto().getLances().get(tamanho - 1);
